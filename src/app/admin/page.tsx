...
export default function AdminPage() {
  ...
  const { squadResults, updateSquadResult, ranking } = useGameStore();
  ...
  const handleUpdate = (nation: string, field: 'wins'|'draws'|'losses'|'groupWin'|'advance', value: number|boolean) => {
  ...
  if (!loggedIn) {
    return (
      <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'var(--font-body)' }}>
        ...
        <p style={{ color:'var(--muted)', fontSize:13, marginBottom:32 }}>Solo per l'amministratore del gioco</p>
        ...
        <a href="/" style={{ color:'var(--muted)', fontSize:12, marginTop:24, textDecoration:'none' }}>← Torna alla classifica</a>
      </div>
    );
  }

  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', color:'var(--text)', fontFamily:'var(--font-body)', paddingBottom:40 }}>
      <div style={{ background:'linear-gradient(135deg, #0A0E1A 0%, #1a1040 100%)', borderBottom:'1px solid var(--border)', padding:'48px 16px 16px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          ...
          <p style={{ color:'var(--muted)', fontSize:12, margin:'4px 0 0' }}>Le modifiche aggiornano la classifica in tempo reale</p>
        </div>
      ...
      {/* Classifica live */}
      <div style={{ margin:'16px 16px 0', padding:14, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12 }}>
        <p style={{ fontFamily:'var(--font-display)', fontSize:14, letterSpacing:1, color:'var(--muted)', margin:'0 0 10px' }}>CLASSIFICA ATTUALE</p>
        {ranking.map((ts, i) => (
          <div key={ts.team.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom: i < ranking.length-1 ? '1px solid var(--border)' : 'none' }}>
            <span style={{ fontSize:13 }}>{['🥇','🥈','🥉'][i]} {ts.team.name}</span>
            <span style={{ fontFamily:'var(--font-display)', fontSize:20, color:'var(--gold)' }}>{ts.totalScore.toFixed(1)}</span>
          </div>
        ))}
      </div>
        ...
        {TEAMS.map(team => (
          <div key={team.id} style={{ marginBottom:24 }}>
            ...
            {team.squads.map(nation => {
              ...
              const coeff = require('@/lib/gameData').COEFFICIENTS[nation] ?? 1;
              const finalScore = pts * coeff;

              return (
                <div key={nation} style={{ background:'var(--surface)', border: isSaved ? '1px solid var(--accent)' : '1px solid var(--border)', borderRadius:12, padding:14, marginBottom:10, transition:'border-color 0.3s' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
                    ...
                    {isSaved && <span style={{ fontSize:11, color:'var(--accent)', background:'rgba(0,200,83,0.1)', padding:'2px 8px', borderRadius:20 }}>✓ Salvato</span>}
                  </div>
                      ...
                      return (
                        <div key={field} style={{ textAlign:'center' }}>
                          ...
                          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
                            <button onClick={() => handleUpdate(nation, field, Math.max(0, r[field]-1))} style={btnStyle}>−</button>
                            <span style={{ fontFamily:'var(--font-display)', fontSize:22, minWidth:24, textAlign:'center' }}>{r[field]}</span>
                            <button onClick={() => handleUpdate(nation, field, r[field]+1)} style={btnStyle}>+</button>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={() => handleUpdate(nation, 'groupWin', !r.groupWin)}
                    ...
                    <button onClick={() => handleUpdate(nation, 'advance', !r.advance)}
